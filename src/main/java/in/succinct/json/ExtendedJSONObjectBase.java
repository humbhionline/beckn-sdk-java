...
public class ExtendedJSONObjectBase extends ExtendedJSONAware<JSONObject> {
    public ExtendedJSONObjectBase(JSONObject value) {
        super(value);
    }

    public ExtendedJSONObjectBase(String payload) {
        super(payload);
    }
    public ExtendedJSONObjectBase(){
        this(new JSONObject());
    }

    private transient Map<String, ObjectHolder<ExtendedJSONAware>> attributeMap = new HashMap<>();

    public <T extends ExtendedJSONAware> T get(Class<T> clazz,String name){
        return get(clazz,name,false);
    }
    public <T extends ExtendedJSONAware> T get(Class<T> clazz,String name,boolean createIfAbsent){
    ...
    public <T extends ExtendedJSONAware> void set(String key, T value){
        if (value == null){
            rm(key);
            return;
        }
    ...
    protected Map<String,ObjectHolder<ExtendedJSONAware>> attributeMap(){
    ...
    public <T extends  ExtendedJSONObjectBase> void update(T from){
        update(from,true);
    }
    public <T extends  ExtendedJSONObjectBase> void update(T from, boolean reset){
        update(from, getObjectCreator(),reset);
    }
    private <T extends  ExtendedJSONObjectBase> void update(T fromSource , ExtendedJSONAwareCreator boCreator){
        update(fromSource,boCreator,true);
    }
    private <T extends  ExtendedJSONObjectBase> void update(T fromSource , ExtendedJSONAwareCreator boCreator, boolean reset){
        if (!hasCommonAncestor(this,fromSource)){
            throw new IllegalArgumentException("Incompatible type of the parameter");
        }

        Class<? extends ExtendedJSONObjectBase> sourceClass = fromSource.getClass();
        ...
        for (Method m : sourceClass.getMethods()) {
            if ((m.getName().startsWith("get") || m.getName().startsWith("is")) && m.getParameterCount() == 0 && !m.getName().equals("getInner") && ExtendedJSONObject.class.isAssignableFrom(m.getDeclaringClass())){
                sourceGetters.put(m.getName(),m);
            }
        }

        for (Method m : getClass().getMethods()) {
            if (!ExtendedJSONObject.class.isAssignableFrom(m.getDeclaringClass())){
                continue;
            }
        ...
        for (Map.Entry<String,Method> entry : sourceGetters.entrySet()) {
            ...
            try {
                ...
                if (targetField == null || reset){
                    if (ExtendedJSONAware.class.isAssignableFrom(targetFieldType)){
                ...
                if (ExtendedJSONObject.class.isAssignableFrom(sourceFieldType) && ExtendedJSONObject.class.isAssignableFrom(targetFieldType)) {
                    if (((ExtendedJSONObject)sourceField).hasAdditionalProperties() || sourceFieldType == targetFieldType){
                        ((ExtendedJSONObject)targetField).setInner(((ExtendedJSONObject)sourceField).getInner());
                    }else {
                        ((ExtendedJSONObject) targetField).update((ExtendedJSONObject) sourceField);
                    }
                } else if (ExtendedJSONObjects.class.isAssignableFrom(sourceFieldType) && ExtendedJSONObjects.class.isAssignableFrom(targetFieldType)){
                    ExtendedJSONObjects sourceFields = (ExtendedJSONObjects) sourceField;
                    ExtendedJSONObjects targetFields = (ExtendedJSONObjects) targetField;
                    for (Object o : sourceFields){
                        if (o instanceof ExtendedJSONObject){
                            ExtendedJSONObject listElement = (ExtendedJSONObject) boCreator.create(o.getClass());
                            if (listElement.getClass() == o.getClass()){
                                listElement.setInner(((ExtendedJSONObject) o).getInner());
                            }else {
                                listElement.update((ExtendedJSONObject) o);
                                if (listElement instanceof ExtendedJSONObjectWithId) {
                                    ExtendedJSONObjectWithId lid = (ExtendedJSONObjectWithId) listElement;
                                    ExtendedJSONObjectsWithId lids = ((ExtendedJSONObjectsWithId) targetFields);
    ...
    public static boolean hasCommonAncestor(Class<?> targetType, Class<?> sourceType) {
        ...
        if (!commonAncestor.isAssignableFrom(sourceType) || commonAncestor == ExtendedJSONObjectBase.class || commonAncestor == Object.class ){
            return false;
        }
    ...
    public static  <T extends ExtendedJSONObjectBase, S extends ExtendedJSONObjectBase> boolean hasCommonAncestor(T target, S source){
        Class<?> commonAncestor = target.getClass();
        Class<?> sourceType = source.getClass();
        while (!commonAncestor.isAssignableFrom(sourceType)){
            commonAncestor = commonAncestor.getSuperclass();
        }
        if (commonAncestor == ExtendedJSONObjectBase.class){
            return false;
        }
        return true;
    }
